...
This activates `.venv` if present, starts the API on `127.0.0.1:8000` without `--reload` (real models), then the Vite client. Ctrl-C stops both.
...
Optional: two terminals from the repo root:
...
```bash
LOST_FOUND_MOCK_ML=1 uvicorn api.main:app --host 127.0.0.1 --port 8000
```

Do not run `uvicorn` with `--reload` or multiple workers during a demo: loaded models live in process memory. The first real request after startup can take minutes while weights load; the Home page shows warmup progress from `GET /api/health`. Set `LOST_FOUND_ALLOW_DEMO_RESET=1` only when you need the Account page "Reload demo" action.
...
- `GET /api/health` (includes `demo_reset_allowed`), `POST /api/health/warmup`
- `POST /api/auth/register|login|logout`, `GET /api/auth/me` (user includes `avatar` hex color), `GET /api/auth/demo-accounts`
- `GET /api/reports?scope=mine|open`, `POST /api/reports` (multipart), `GET /api/reports/{id}`, `GET /api/reports/{id}/images/{n}`, `PATCH /api/reports/{id}/contact`, `PATCH /api/reports/{id}/status` (`{status: "closed"}`, owner only)
- `GET /api/matches?report_id=` (cached ranking with component scores and `visual` evidence; omits dismissed pairs), `PATCH /api/matches/dismiss` (`{lost_report_id, found_report_id}`, either party)
- `GET /api/notifications` (`{notifications, unread_count}`, unread first), `POST /api/notifications/{id}/read`, `POST /api/notifications/read-all`
...
- Photos: `rembg` crops the object from the background, DINOv2 ViT-S/14 (`facebook/dinov2-small`) shortlists the top 5 visually similar pairs, then LightGlue + ALIKED (`kornia`) re-ranks them. The left crop is matched against the right crop at 0°/90°/180°/270° (stopping early if 0° already has a strong inlier count), using homography RANSAC first and the fundamental matrix if needed. If ALIKED still finds no geometry, SIFT+LightGlue runs once. Photo score is 0.35 × DINOv2 cosine + 0.65 × feature-match when LightGlue finds inliers, or DINOv2 cosine alone when it does not. Pairs outside the shortlist have no photo score.
